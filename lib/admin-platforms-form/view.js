...
export default class PlatformForm extends FormView {
  ...
  switchOn() {
    ...
    this.bind('keyup', '#platform-options-search', this.bound('onsearchchange'));
  ...
  renderSelecting() {
    platformStore.findAll().then(platforms => {
      ...
      var searchBox = o('#platform-options-search');
  ...
  filterOptions () {
    ...
    container.find('*').each(opt => {
      var id = opt.attr('value');
      var mediaTitle = opt.find('.platform-title').text();
      var query = searchBox.val();

      if (query != '') {
        if (mediaTitle.indexOf(query < 0)) opt.addClass('hide');
      } else {
        opt.removeClass('hide');
      }

      if ((this.platform) && id == this.platform.id) opt.addClass('hide');
    });
  }
  ...
  initializeDragging() {
    ...
    var drake = dragula([operations, options, platformSelect], {
      ...
      accepts: (el, target) => {
        if (target === operations) return true;
        if (target == platformSelect) return this.acceptsFunction(el, target);
        return false;
      },
  ...
  acceptsFunction (el, target) {
    ...
    var good = true;
    if (this.platform && p.platformTree) {
      good = (p.platformTree.string.indexOf(this.platform.id) < 0);
    }

    if (good) return (target !== operations || target !== options);

    confirm(t('admin-platforms-form.illegal-operation'), t('admin-platforms-form.warning.infinite-loop') + p.mediaTitle)
      .ok(t('ok'))
      .modal()
      .closable()
      .effect('slide')
