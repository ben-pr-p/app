...
export default class PlatformForm extends FormView {
  ...
  renderSelecting() {
    platformStore.findAll().then(platforms => {
      ...
      options.filter(opt => opt.id != this.platform.id).forEach(opt => {
  ...
  initializeDragging() {
    ...
    function refreshHiddenInput(el) {
      var hiddeninput = o('[name="platformTree"]');
      hiddeninput.val(o('#platformTree').find('.option').map(opt => o(opt).attr("value")).join('|'));
    }

    drake.on('drop', refreshHiddenInput);
    drake.on('remove', refreshHiddenInput);
  }
}
