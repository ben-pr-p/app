...
page(forumRouter('/admin/platforms/create'), findAllTags, ctx => {
  ctx.sidebar.set('platforms');
  // render new platform form
  let form = new PlatformForm(null, ctx.forum, ctx.tags);
...
page(forumRouter('/admin/platforms/:id'), findAllTags, findPlatform, ctx => {
  // force section for edit
  // as part of list
  ctx.sidebar.set('platforms');

  // render platform form for edition
  let form = new PlatformForm(ctx.platform, ctx.forum, ctx.tags);
  form.replace('.admin-content');
