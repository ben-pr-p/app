...
import PlatformListView from '../admin-platforms/view.js';
import PlatformForm from '../admin-platforms-form/view.js';
...
if (config.platforms) {
  require('../admin-platforms/admin-platforms.js');
  require('../admin-platforms-form/admin-platforms-form.js');
}
...
function valid(ctx, next) {
  ...
  if (/topics|tags|platforms|users/.test(section)) return next();
  if (/topics|tags|platforms|users\/create/.test(section)) return next();
  if (/topics|tags|platforms|users\/[a-z0-9]{24}\/?$/.test(section)) return next();
}
