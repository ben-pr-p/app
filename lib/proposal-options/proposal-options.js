...
export default class ProposalOptions extends View {
  ...
  onvoting (value) {
    ...
    switch (value) {
      case 'negative':
        this.proposal.downvotes.push({author: user.id, platform: null});
        el = dom(negative);
        break;
      case 'positive':
        this.proposal.upvotes.push({author: user.id, platform: null});
        el = dom(positive);
        break;
      case 'neutral':
        this.proposal.abstentions.push({author: user.id, platform: null});
        el = dom(neutral);
        break;
    }
  ...
  unvote () {
    var upvoteidx = this.proposal.upvotes.map(function (v) {return v.author}).indexOf(user.id)
    if (~upvoteidx) {
      this.proposal.upvotes.splice(upvoteidx, 1);
    }

    var downvoteidx = this.proposal.downvotes.map(function (v) {return v.author}).indexOf(user.id)
    if (~downvoteidx) {
      this.proposal.downvotes.splice(downvoteidx, 1);
    }

    var abstentionidx = this.proposal.abstentions.map(function (v) {return v.author}).indexOf(user.id)
    if (~abstentionidx) {
  ...
  renderVoted () {
    ...
    var census = this.proposal.abstentions.concat(this.proposal.upvotes).concat(this.proposal.downvotes);
    var votedidx = census.map(function (v) {return v.author}).indexOf(user.id);

    if (~votedidx) {
      if (census[votedidx].platform) {
        platformStore.findOne(census[votedidx].platform).then(p => {
          var el = dom(voted, { proposal: this.proposal, platform: p.mediaTitle });
          meta[0].insertBefore(el, meta[0].firstChild);
        });
      } else {
        var el = dom(voted, { proposal: this.proposal });
        meta[0].insertBefore(el, meta[0].firstChild);
      }
    }
  }
