...
export default class NodeEdit extends view('appendable', 'emptyable', 'withEvents') {
  constructor (options) {
    ...
    var body = options.locals.node.body;
    if (body) body = body.replace('</br></br>', '\n');
    options.locals.editBody = body;
  ...
  onsubmit (ev) {
    ev.preventDefault();
    this.node.body = dom(this.el).find('.node-edit-body').val().replace('\n', '</br></br>');
