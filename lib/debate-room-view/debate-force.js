...
export default class DebateForce {
  constructor (options) {
    ...
    var tobind = [
      ...
      'keydown', 'keyup', 'resetMouseVars',                                                       // global event handlers
  ...
  initialize () {
    ...
    this.force = d3.layout.force()
      ...
      .linkDistance((d) => {
        return 300 * scalingFactor(d.target.depth, 5);
      })
      .charge(-500)
      .gravity(0)
      .on('tick', this.tick);
  ...
  tick (ev) {
    ...
    this.path.attr('d', (d) => {
      ...
      let rect = {
        x: d.target.x,
        y: d.target.y,
        w: d.target.depth ? scalingFactor(d.target.depth, 5) * targetBox.offsetWidth : targetBox.offsetWidth,
        h: d.target.depth ? scalingFactor(d.target.depth, 5) * targetBox.offsetHeight : targetBox.offsetHeight
      };
...
function calcMovement(lineOrigin, rect) {
  // uses the center of the rect for x and y
