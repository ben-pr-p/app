...
export default class DebateForce {
  ...
  selectNode (node) {
    ...
    d3.selectAll('.node-card').classed('selected', d => d == this.selectedNode);

    this.selectedEdge = null;
    this.path.selectAll('path').classed('selected', false);
  }

  selectEdge (edge) {
    this.selectedEdge = edge;
    this.path.selectAll('svg:path').classed('selected', d => d == this.selectedEdge);
