 ...
 *   - initialize comments to good place *** important ***
 ...
 *   - delete comment
 *   - change edge type
 *   - select node with thicker border and bring to front
 *   - edge labels
 *   - figure out how to zoom with something
 */
...
export default class DebateForce {
  ...
  initialize () {
    ...
    this.force = d3.layout.force()
      ...
      .linkDistance((d) => {
        return 200 * this.scaleDepth(d.source.depth);
      })
      .linkStrength(1)
      .charge(-1000)
      .chargeDistance(200)
  ...
  onnodeclick (node, d) {
    ...
    if (this.mousedownNode === this.selectedNode) {
      this.selectedNode = null;
      d3.select(node).select('.node-card').classed('selected', false);
    } else {

      this.selectedNode = this.mousedownNode;
      d3.selectAll('.node-card').classed('selected', false);
      d3.select(node).select('.node-card').classed('selected', true);
