 ...
 */
const standardNodeHeight = 78; // standard node height
const sld = 500; // standard link distance

const updateInterval = 10000;
const updateOn = false;

const standardOpacity = 0.6;
const standardStrokeWidth = 4;
...
export default class DebateForce {
  ...
  initialize () {
    ...
    this.force = d3.layout.force()
      ...
      .linkDistance((d) => {
        return sld * this.scaleDepthWeak(d.source.depth);
      })
      .linkStrength(1)
      .charge(-500)
      .gravity(0)
    ...
    this.controls = d3.select('.controls')
      .style('left', this.svg.node().getBoundingClientRect().left + ctrlLeft)
      .style('top', this.svg.node().getBoundingClientRect().top + ctrlTop);
  ...
  tick (ev) {
    ...
    this.path.attr('d', (d) => {
      ...
      let targetRect = {
        ...
        h: d.target.depth ? (this.scaleDepthWeak(d.target.depth) * standardNodeHeight) : standardNodeHeight
      };
    ...
    this.fo.attr('transform', function (d) {
      ...
      let y = d.y - self.scaleDepthWeak(d.depth) * (standardNodeHeight / 2);
  ...
  restart () {
    ...
    this.path.enter()
      .append('svg:path')
        ...
        .style('stroke-width', (d) => `${standardStrokeWidth * this.scaleDepthWeak(d.target.depth)}px`)
  ...
  createEdge (edge) {
    edge.referenceType = this.debate.referenceType;
    edge.reference = this.debate.reference;
  ...
  createNodeWithEdge (node, edge) {
    node.referenceType = this.debate.referenceType;
    node.reference = this.debate.reference;
  ...
  onsearchchange () {
    ...
    if (contents == '') {
      this.path.style('opacity', standardOpacity);
  ...
  calcIdeal (node) {
    ...
    if (edgeFrom) {
      ...
      if (angle != null) {
        node.ix = p.ix + Math.cos(angle / 180 * Math.PI) * sld * this.scaleDepthWeak(node.depth);
        node.iy = p.iy + Math.sin(angle / 180 * Math.PI) * sld * this.scaleDepthWeak(node.depth);
      }
    }
    ...
    switch (d3.event.keyCode) {
      ...
      case 40: // down arrow
        if (this.selectedNode) {
          ...
          var nodes = this.nodes.filter(n => n != this.selectedNode);
          nodes.forEach(n => {
            // calculate the actual angle for each edge
            let xdist = this.selectedNode.x - n.x;
            let ydist = this.selectedNode.y - n.y;
            let slope = (xdist) / (ydist);
            let mag = Math.sqrt(Math.pow(xdist, 2) + Math.pow(ydist, 2));
            // ranking formula is (angle - goal angle) + (mag * magWeight)
            let a = Math.atan(slope) * (180 / Math.PI);
            n.score = Math.abs(a - arrowAngles[d3.event.keyCode]) + (mag * magWeight);
          });
      ...
      case 13:
        bus.emit(`enter-on-${this.selectedNode.id}`);
        break;
    }
  }
  ...
  selectNode (node) {
    ...
    d3.selectAll('.node-card').classed('keyselected', d => d == this.selectedNode);

    this.selectedEdge = null;
    this.path.classed('keyselected', false);
  }

  selectEdge (edge) {
    this.selectedEdge = edge;
    this.path.classed('keyselected', d => d == this.selectedEdge);
  ...
  update () {
    ...
    var query = {type: this.debate.referenceType, id: this.debate.reference};
