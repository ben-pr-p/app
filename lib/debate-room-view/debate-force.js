...
export default class DebateForce {
  ...
  initialize () {
    ...
    this.force = d3.layout.force()
      ...
      .linkDistance((d) => {
        return 300 * this.scaleDepthWeak(d.source.depth);
      })
      .linkStrength(1)
      .charge(-1000)
      .chargeDistance(200)
  ...
  tick (ev) {
    ...
    // apply custom edge based forces
    for (var i = this.edges.length - 1; i >= 0; i--) {
      let d1 = this.edges[i];
      let l1 = Line.create([d1.source.x, d1.source.y], [d1.target.x, d1.target.y]);

      for (var j = this.edges.length - 1; j >= 0; j--) {
        let d2 = this.edges[j];
        let l2 = Line.create([d2.source.x, d2.source.y], [d2.target.x, d2.target.y]);

        if (l1.intersects(l2)) {
          // get lowest depth element
          let lowest = d1.source;
          [d1.source, d1.target, d2.source, d2.target].forEach(n => {
            if (n.depth < lowest.depth) lowest = n;
          });
          // ok, now n is the one we want to move
          // if it's above the center, move it above everything
          if (n.y > this.svg.node().offsetHeight / 2) {
            n.y = d3.max(this.nodes, node => node.y);
          } else {
            n.y = d3.min(this.nodes, node => node.y);
          }
        }
      };
    };
    ...
    this.fo.attr('transform', function (d) {
      ...
      // to do - be averse to edge overlap
