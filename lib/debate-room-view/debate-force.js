...
export default class DebateForce {
  ...
  calcIdeal (node) {
    ...
    if (edgeFrom) {
      ...
      node.ix = p.ix + Math.cos(angle / 180 * Math.PI) * 400 * this.scaleDepthStrong(node.depth);
      node.iy = p.iy + Math.sin(angle / 180 * Math.PI) * 400 * this.scaleDepthStrong(node.depth); 
      console.log(node.ix);     
    }
