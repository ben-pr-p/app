...
app.get('/node/updates', findForum, function(req, res) {
  ...
  if (!req.query.since) {
    log("Request didn't include `since` parameter");
    return res.send(400);
  }
  log('Request /node/updates for %s %s since %s', req.query.type, req.query.id, req.query.since);

  api.debateroom.nodeUpdates({reference: req.query.id, referenceType: req.query.type}, req.query.since, (err, debateDoc) => {
    if (err) return _handleError(err, req, res);

    log('Serving node updates for %s %s since %j', req.query.type, req.query.id, req.query.since);
    res.json(debateDoc);
  });
});
...
app.get('/edge/updates', findForum, function(req, res) {
  ...
  log('Request /edge/updates for %s %s since %s', req.query.type, req.query.id, req.query.since);

  api.debateroom.edgeUpdates({reference: req.query.id, referenceType: req.query.type}, req.query.since, (err, debateDoc) => {
    if (err) return _handleError(err, req, res);

    log('Serving edge updates for %s %s since %j', req.query.type, req.query.id, req.query.since);
    res.json(debateDoc);
  });
});
