...
function exposeNode(nodeDoc) {
  var nodeListKeys = [
    'id type body title author votes',
    'createdAt editedAt',
    'debateType reference referenceType forum',
 ...
 * 
 */
app.get('/debateroom/all', findForum, function(req, res) {
  if (!req.query.type) {
 ...
 * 
 */
app.get('/node/updates', findForum, function(req, res) {
  if (!req.query.type) {
    log("Request didn't include `type` parameter");
    return res.send(400);
  }
  ...
  log('Request /node/updates for %s %s', req.query.type, req.query.id);

  api.debateroom.searchNodes({reference: req.query.id, referenceType: req.query.type}, (err, debateDoc) => {
    if (err) return _handleError(err, req, res);

    log('Serving node updates for %s %s', req.query.type, req.query.id);
    res.json(debateDoc);
  });
});
 ...
 * 
 */
app.get('/edge/updates', findForum, function(req, res) {
  if (!req.query.type) {
    log("Request didn't include `type` parameter");
    return res.send(400);
  }
  ...
  log('Request /edge/updates for %s %s', req.query.type, req.query.id);

  api.debateroom.searchEdges({reference: req.query.id, referenceType: req.query.type}, (err, debateDoc) => {
    if (err) return _handleError(err, req, res);

    log('Serving edge updates for %s %s', req.query.type, req.query.id);
    res.json(debateDoc);
  });
});
 ...
 * 
 */
app.get('/node/:id', findForum, function(req, res) {
 ...
 * 
 */
app.delete('/node/:id', restrict, function (req, res) {
  log('Request DELETE /node/%s', req.params.id);

  api.debateroom.removeNode(req.params.id, function (err, nothing) {
    if (err) return _handleError(err, req, res);

    log('Deleted node %s', req.params.id);
    res.send(200);
  })
});
 ...
 * 
 */
app.post('/node/:id/vote', restrict, function (req, res) {
  log('Request POST /node/%s/vote', req.params.id);
  ...
  api.debateroom.voteNode(req.params.id, vote, function (err, node) {
    if (err) return _handleError(err, req, res);

    log('Voted %s on node %s', req.body.value, req.params.id);
    res.json(exposeNode(node));
  })
});
