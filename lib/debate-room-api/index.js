...
var argListKeys = [
  'id type text parents children author',
  'createdAt editedAt'
].join(' ');
...
var debateTypeListKeys = [
  'id name argumentTypes linkTypes'
].join(' ');
...
app.get('/debateroom/all', findForum, function(req, res, next) {
  ...
  log('Request /debateroom/for %s %s', req.query.type, req.query.id);

  api.debateroom.searchDebate(req.query.id, req.query.type, (err, debateDoc) => {
    if (err) return _handleError(err, req, res);

    if (!debateDoc) {
      log('Debate not found – serving null response');
      return res.json(null);
    }

    log('Serving debate %s', debateDoc.id);
    res.json(exposeDebate(debateDoc));
  });
});

app.get('/debateroom/:id', findForum, function(req, res, next) {
  log('Request /debateroom/:id', req.params.id);

  api.debateroom.getDebate({id: req.params.id}, function(err, debateDoc) {
    if (err) return _handleError(err, req, res);
...
app.post('/debateroom/create', findForum, function(req, res, next) {
  if (!req.query.type) {
    log("Request didn't include `type` parameter");
    return res.send(400);
  } 
  ...
  var typeApi = (req.query.type == 'topic') ? api.topic : api.platform;
  typeApi.get(req.query.id, (err, item) => {
    if (err) return _handleError(err, req, res);

    if (!item) return res.send(400);

    item.text = (req.query.type == 'topic') ? item.bodyTruncationText : item.body;
...
app.get('/debateroom/argument/:id', function (req, res) {
...
app.post('/debateroom/argument/create', restrict, function (req, res) {
...
app.post('/debateroom/argument/:id', restrict, function (req, res) {
...
app.get('/debateroom/type/:id', restrict, function (req, res) {
