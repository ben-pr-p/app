...
export default class EventHelp {
  constructor(VennDiagram, DataHelp, width, height) {
    this.vd = VennDiagram;
    this.dh = DataHelp; 
   ...
   */
  mouseover (d, i) {
    venn.sortAreas(d3.select('#diagram-container'), d); 
  ...
  onclick (d, i) {
    ...
    Promise
    .all(opinions.map( o => this.vd.getTopic(o.topicId)))
    .then( topics => {
      ...
      if (platformTree && platformTree.string) {
