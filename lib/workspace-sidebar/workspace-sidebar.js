...
class Sidebar extends view('appendable') {
  ...
  refresh (topics, topicFilter, platforms, platformFilter) {
    this.refreshTopicSection(topics, topicFilter);
    this.refreshPlatformSection(platforms, platformFilter);
  }

  refreshPlatformSection (platforms, platformFilter) {
    this.refreshPlatformList(platforms);
    this.refreshPlatformFilter(platformFilter);
  }

  refreshTopicSection (topics, topicFilter) {
    this.refreshTopicList(topics);
    this.refreshTopicFilter(topicFilter);
  }
  ...
  refreshPlatformFilter (filter) {
    if (this.platformFilterView) {
      if (equals(this.platformFilterView.options.locals, filter)) return;
      this.platformFilterView.remove();
    }
    if (filter) {
  ...
  refreshTopicFilter (filter) {
    if (this.topicFilterView) {
      if (equals(this.topicFilterView.options.locals, filter)) return;
      this.topicFilterView.remove();
    }
    if (filter) {
      console.log('im here')
  ...
  setDragging () {
    var drake = dragula([document.querySelector('.sidebar-topic-list')], {
