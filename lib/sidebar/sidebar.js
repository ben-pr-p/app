...
class Sidebar extends view('appendable', 'withEvents') {
  ...
  onsidebarclick (e) {
    ...
    var prefix = this.forum ? this.forum.url.slice(0, this.forum.url.length - 1) : '';
    this.show(selection);
    page(prefix + urls[selection]);
  }
  ...
  show (sidebar) {
    ...
    dom('.sidebar-icon').removeClass('active');
    dom(`[data-sidebar-name="${sidebar}"]`).addClass('active');
  }
