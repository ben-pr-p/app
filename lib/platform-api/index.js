...
var platformListKeys = [
  'id mediaTitle status forum',
  'createdAt',
  'deletedAt votable author'
].join(' ');
...
app.get('/platform/all', findForum, function(req, res, next) {
  ...
  api.platform.all({forum: req.forum}, function(err, platforms) {
    ...
    res.json(platforms.map(function(platformDoc){
      return exposePlatform(platformDoc, req.user, platformListKeys);
    }));
...
//   log('Request GET /set/%s', req.params.id);

//   api.set.get(req.params.id, function (err, set) {
//     if (err) return _handleError(err, req, res);
...
app.post('/platform/create', restrict, hasAccess, function (req, res) {
  ...
  api.platform.create({author: req.user}, function (err, platform) {
    if (err) {
      log('Found error %j', err);
      res.send(err);
    }

    res.json(exposePlatform(platform, req.user, platformListKeys));
  });
});
