...
/**
 * [voteNode description]
 * @param  {[type]}   id   [description]
 * @param  {[type]}   vote [description]
 * @param  {Function} fn   [description]
 * @return {[type]}        [description]
 */
exports.voteNode = function (id, vote, fn) {
  exports.getNode(id, (err, node) => {
    if (err) {
      log(err);
      return fn(err);
    ...
    if (node.author.id == vote.caster.id) return fn(new Error("User tried to vote own comment"));
