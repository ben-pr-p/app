...
/**
 * [nodeUpdates description]
 * @param  {[type]}   options. since         [description]
 * @param  {Function} fn       [description]
 * @return {[type]}            [description]
 */
exports.nodeUpdates = function (options, since, fn) {
  var query = options;
  var sinceDate = new Date(since);
  query['$or'] = [ {createdAt: {$gt: sinceDate} }, {editedAt: {$gt: sinceDate} } ];

  exports.searchNodes(query, (err, nodes) => {
    if (err) return fn(err);

    return fn(null, nodes);
  });
}

/**
 * [edgeUpdates description]
 * @param  {[type]}   options. since         [description]
 * @param  {Function} fn       [description]
 * @return {[type]}            [description]
 */
exports.edgeUpdates = function (options, since, fn) {
  var query = options;
  var sinceDate = new Date(since);
  query['$or'] = [ {createdAt: {$gt: since} }, {editedAt: {$gt: since} } ];

  exports.searchEdges(query, (err, edges) => {
    if (err) return fn(err);

    return fn(null, edges);
  });
}
