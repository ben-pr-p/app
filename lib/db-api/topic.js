...
exports.vote = function vote(id, params, fn) {
  var query = { _id: id, deletedAt: null };

  log('Proceding to vote %s at topic %s by user %s from platform %s', params.value, id, params.user.id || user, params.platform);
  ...
  .exec(function (err, topic) {
    ...
    doVote(topic, params, fn);
  });
};

function doVote(topic, params, cb) {
  topic.vote(params, function (err) {
    if (err) {
      log('Found error %s', err);
      return cb(err);
    }

    log('Voted %s at topic %s by user %s from platform %s', params.value, params.topic.id, params.user.id || params.user, params.platform);
    cb(null, topic);
  });
}

/**
 * Undo platform vote topic
 * 
 * @param {String} id Topic `id`
 * @param {String} user author of the vote
 ...
 * @param {Function} fn callback function
 */
exports.unvoteplatform = function unvoteplatform(id, user, platform, fn) {
  var query = { _id: id, deletedAt: null };

  log('Proceeding to remote votes with platform %s from topic %s', platform, id);
  ...
  .exec(function (err, topic) {
    ...
    topic.unvoteplatform(user, platform, fn);
  });
}
