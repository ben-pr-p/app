...
exports.vote = function vote(id, user, value, platform, fn) {
  var query = { _id: id, deletedAt: null };

  log('Proceding to vote %s at topic %s by user %s from platform %s', value, id, user.id || user, platform);
  ...
  .exec(function (err, topic) {
    ...
    doVote(topic, user, value, platform, fn);
  });
};

function doVote(topic, user, value, platform, cb) {
  topic.vote(user.id, value, platform, function (err) {
    if (err) {
      log('Found error %s', err);
      return cb(err);
    }

    log('Voted %s at topic %s by user %s from platform %s', value, topic.id, user.id || user, platform);
    cb(null, topic);
  });
}
