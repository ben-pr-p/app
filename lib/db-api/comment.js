 ...
 * 
 * @param  {String}   type    [either topic or platform]
 * @param  {ObjectId} itemId  [objectId of either topic or platform]
 * @param  {Function} fn      [callback]
 * @return {Module}           [`comment` module]
 */
exports.comments = function getAll(type, itemId, paging, fn) {
  log('Looking for all comments from %s %s', type, itemId);
  query = { type: type, itemId: itemId, context: { $ne: 'paragraph' }};
  paging = paging || {};
...
/**
 * Get all comments by a user for a particular topic or platform
 * 
 * @param  {String}   type   [either topic or platform]
 * @param  {ObjectId} itemId [objectId of either topic or platform]
 ...
 * @return {Module}          [`comment` module]
 */
exports.userComments = function userComments(type, itemId, userId, fn) {
  log('Looking for all comments from %s %s by %s', type, itemId, userId);
  query = {type: type, itemId: itemId, author: userId, context: { $ne: 'paragraph' } };
...
/**
 * Create comment for `topic` or `platform` by `author`
 * with `text`
 *
...
exports.getSideComments = function getSideComments (type, itemId, paging, fn) {
  log('Querying side comments for %s %s', type, itemId);
  paging = paging || {};

  Comment
    .find({ type: type, itemId: itemId, context: 'paragraph' })
    .sort(paging.sort || '-createdAt')
    .skip(paging.page * paging.limit)
    .limit(paging.limit)
    .exec(function (err, comments) {
