 ...
 */
exports.all = function all(options, cb) {
  var query = {};
  var paging = {};
  ...
  if (typeof topicId == 'function') {
    ...
    var query = {context: { $ne: 'paragraph' }};
    if (options.type) query.type = options.type;
    if (options.itemId) query.itemId = options.itemId;
    if (options.userId) query.author = options.userId;
    if (options.context) query.context = options.context;

    log('Looking for all comments matching query %j', query);
