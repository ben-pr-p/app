...
var opinionApi = require('./opinion');
...
function doOpine(platform, topicId, value, cb) {
  opinionApi.ensureOpinion( {topicId: topicId, value: value}, function (err, opinion) {
    if (err) return cb(err);

    log('Proceeding to add opinion %s to platform %s', opinion.id, platform.id);
    platform.opine(opinion, function (err) {
      if (err) {
        log('Found error %s', err);
        return cb(err);
      }
