...
exports.all = function (params, fn) {
  ...
  Platform
  .find(query)
  .populate('opinions platformTree tag')
...
exports.extendedOpinions = function extendedOpinions(id, fn) {
  Platform
  .findOne({ _id: id, deletedAt: null })
  .populate('directOpinions platformTree tag')
  .exec(function(err, platform) {
    if (err) return fn(err);
    if (!platform) return fn(null);
...
exports.update = function update(id, data, fn) {
  ...
  function onget(err, platform) {
    ...
    ensureTag(data, (err, data) => {
      ptApi.update(platform.id, platform.platformTree.id, data.platformTree, (err, pt) => {
        if (err) return fn(err);
  ...
  function onupdate(err, platform) {
    ...
    ptApi.get(platform.platformTree, (err, pt) => {
      platform.platformTree = pt;
      return fn(err, platform);
    });
  }
...
/**
 * a.removeConflicts(b) will return a with elements in b that have identical
 * topicIds removed
 * 
 * @param  {Array} other  [b]
 * @return {Array}        [filtered this]
 */
