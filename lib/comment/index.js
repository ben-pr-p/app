...
app.get('/comment/all', function (req, res) {
  ...
  log('Request /comment/all for %s %s', req.query.type, req.query.id);
  ...
  api.comment.comments(req.query.type, req.query.id, paging, function (err, comments) {
    if (err) return _handleError(err, req, res);

    if (!req.query.count) {
      log('Serving %s %s comments %j', req.query.type, req.query.id, pluck(comments, 'id'));
      ...
      log('Serving %s %s comments count: %d', req.query.type, req.query.id, comments.length);
...
app.get('/comment/my-comments', restrict, function (req, res) {
  ...
  log('Request /comment/my-comments for %s %s', req.query.type, req.query.id);

  api.comment.userComments(req.query.type, req.query.id, req.user.id, function (err, comments) {
    if (err) return _handleError(err, req, res);

    log('Serving %s %s comments %j for user %s', req.query.type, req.query.id, pluck(comments, 'id'), req.user.id);
...
app.post('/comment/comment', restrict, function (req, res) {
  ...
  log('Request /comment/comment for %s %s with %j', req.query.type, req.query.id, req.body.comment);
  
  var comment = {
    text: req.body.text,
    context: req.body.context || req.query.type,
...
app.get('/comment/sidecomments', function (req, res) {
  ...
  log('Requesting /%s/%s/sidecomments', req.query.type, req.query.id);

  api.comment.getSideComments(req.query.type, req.query.id, null, function (err, comments) {
    if (err) return _handleError(err, req, res);
    log('Serving %s %s body\'s comments %j', req.query.type, req.query.id, pluck(comments, 'id'));
