...
class platformStore extends Store {
  ...
  opinions (id) {
    if (!this.item.get(id)) {
      return Promise.reject(new Error('Cannot get opinions for not fetched item.'));
    }

    let promise = new Promise((resolve, reject) => {
      request
        .post(`${this.url(id)}/opinions`)
        .end((err, res) => {
  ...
  unvote (id) {
    if (!this.item.get(id)) {
      return Promise.reject(new Error('Cannot unvote not fetched item.'));
    }

    let promise = new Promise((resolve, reject) => {
      request
        .post(`${this.url(id)}/unvote`)
        .end((err, res) => {
          if (err || !res.ok) return reject(err);
