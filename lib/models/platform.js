...
PlatformSchema.methods.opine = function (topic, value, cb) {
  var opinion = { topicId : topic, value: value };

  this.unopine(topic, value, function (err, platform) {
    if (err) {
...
PlatformSchema.methods.unopine = function (topic, value, cb) {
  var opinions = this.opinions;

  var conflicts = opinions.filter(function(o) {
    return (o.topicId == topic);
  });
