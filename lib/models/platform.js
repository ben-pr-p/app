...
var PlatformSchema = new Schema({
  ...
  , directOpinions: [OpinionSchema]
  , extendedOpinions: [OpinionSchema]
  ...
  , platformTree: {type: ObjectId, ref: 'PlatformTree'}
});

PlatformSchema.methods.vote = function (user, cb) {
  this.participants.addToSet(user);
  if (cb) this.save(cb);
}

PlatformSchema.methods.unvote = function (user, cb) {
  var useridx = this.participants.indexOf(user);
  this.participants.splice(useridx, 1);
  if (cb) this.save(cb);
}
...
PlatformSchema.methods.opine = function (opinion, cb) {
  this.unopine(opinion.topicId, function (err, platform) {
    ...
    platform.directOpinions.push(opinion);
...
PlatformSchema.methods.unopine = function (topicId, cb) {
  var opinions = this.directOpinions;
