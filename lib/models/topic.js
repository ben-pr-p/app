...
TopicSchema.methods.vote = function(params, cb) {
  var user = params.user, value = params.value, platform = params.platform;
  ...
  var vote = { author: user, value: value, caster: user, platform: platform };
...
/**
 * Unvote Topic from provided user 
 * if it's a platform vote from the provided platform
 * @param {User|ObjectId|String} user
 * @param {Platform|ObjectId|String} user
 * @param {Function} cb
 * @api public
 */

TopicSchema.methods.unvoteplatform = function(user, platformId, cb) {
  var votes = this.votes;
  var c = user.get ? user.get('_id') : user;

  var voted = votes.filter(function(v) {
    var a = v.author.get ? v.author.get('_id') : v.author;
    if (!a.equals ? a.equals(c) : a === c) return false;

    return platformId == v.platform;
  });
